...
def blog_list_common_data(requests, blogs_all_list):
    ...
    all_pages = paginator.num_pages  # 总页数
    if all_pages < 5:
        page_range = list(
...
# 根据类型筛选
def blog_with_type(requests, blog_type_pk):
...
# 根据时间筛选
def blog_with_date(requests, year, month):
    blog_all_list = Blog.objects.filter(created_time__year=year, created_time__month=month)  # 获取全部博客
    context = blog_list_common_data(requests, blog_all_list)
    context['blog_with-date'] = '{}年{}月'.format(year, month)
